...
const App = () => {
  ...
  const renderContent = () => {
    switch(currentPage) {
      ...
      case 'finance': return <Finance userRole={user.role} />;
      case 'info': return <InfoPublik userRole={user.role} userName={user.full_name} />;
      case 'aspirasi': 
    return <Aspirasi 
        userRole={user.role} 
        userId={user.id}
            ...
            <div className="flex items-center gap-4">
               ...
               <div>
                  <h2 className="text-2xl md:text-3xl font-extrabold text-gray-800 tracking-tight truncate">
                     {currentPage === 'dashboard' ? 'Overview' : currentPage.toUpperCase()}
                  </h2>
